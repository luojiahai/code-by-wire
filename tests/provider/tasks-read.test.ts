...
function writeTask(home: string, sessionId: string, file: string, body: object): void {
...
describe('provider.readTasks', () => {
  it('returns changed tasks, then unchanged for the echoed token', () => {
    const home = makeHome()
    writeTask(home, 'sid', '1.json', { id: '1', subject: 'A', status: 'pending', blocks: [], blockedBy: [] })
  ...
  it('is absent for a session with no tasks dir', () => {
    expect(createClaudeProvider({ claudeDir: makeHome() }).readTasks('nope').status).toBe('absent')
  })
})
