...
- `pnpm dev` runs the app (electron-vite). `pnpm test` runs the suite (vitest); tests live in `tests/` mirroring the source tree. `tests/renderer/**` runs under jsdom (renderer logic only); there's no component-render harness, so verify actual UI by hand. Vitest only picks up `tests/**/*.test.ts` — no `.tsx` tests.
