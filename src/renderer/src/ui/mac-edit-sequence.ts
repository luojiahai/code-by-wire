 ...
 *  callers can pass the event straight through, and tests can pass plain objects. */
...
/**
 * Translate a macOS editing keystroke into the readline control bytes a readline-style prompt
 * understands (the Claude Code CLI prompt, or a real shell's own line editor — both read the same
 * bytes), or null to let xterm handle the key untouched.
 *
 * xterm's own arrow-modifier sequences (`\x1b[1;3D` for option+arrow, `\x1b[1;5D` for ctrl+arrow)
 * aren't interpreted by a readline-style line editor, so a raw xterm terminal leaves option+arrow
 * inert. We send the raw readline bytes instead. Only keydown is translated. Shift combos (selection
 * — not a thing in a readline prompt), ctrl combos, ambiguous cmd+option, keys mid-IME-composition,
 * and everything unmapped return null.
 */
